...
BuildDocker() {
  PrepareBuildDocker
  go build -o ./bin/stm -ldflags="$ldflags" -tags=jsoniter .
}
...
BuildDockerMultiplatform() {
  ...
  for i in "${!OS_ARCHES[@]}"; do
    ...
    go build -o build/$os/$arch/stm -ldflags="$docker_lflags" -tags=jsoniter .
  done
  ...
  for i in "${!DOCKER_ARM_ARCHES[@]}"; do
    ...
    go build -o build/${docker_arch%%-*}/${docker_arch##*-}/stm -ldflags="$docker_lflags" -tags=jsoniter .
  done
}
...
BuildRelease() {
  rm -rf .git/
  mkdir -p "build"
  BuildWinArm64 ./build/stm-windows-arm64.exe
  xgo -out "$appName" -ldflags="$ldflags" -tags=jsoniter .
  # why? Because some target platforms seem to have issues with upx compression
  upx -9 ./stm-linux-amd64
  cp ./stm-windows-amd64.exe ./stm-windows-amd64-upx.exe
  upx -9 ./stm-windows-amd64-upx.exe
  mv stm-* build
}
...
MakeRelease() {
  ...
  for i in $(find . -type f -name "$appName-linux-*"); do
    cp "$i" stm
    tar -czvf compress/"$i".tar.gz stm
    rm -f stm
  done
  ...
  for i in $(find . -type f -name "$appName-windows-*"); do
    cp "$i" stm.exe
    zip compress/$(echo $i | sed 's/\.[^.]*$//').zip stm.exe
    rm -f stm.exe
  done
  cd compress
